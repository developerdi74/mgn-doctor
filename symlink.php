...
if($_POST['path']) $path=rtrim($_POST['path'], "/\\");
	...
	$path='../site1/www';
if($_POST['create']){
	if(preg_match("#^/#", $path)) $full_path=$path;
	else
		$full_path=realpath($_SERVER['DOCUMENT_ROOT'].'/'.$path);
	if(file_exists($_SERVER['DOCUMENT_ROOT']."/bitrix")) $strError="В текущей папке уже существует папка bitrix";
...
//					if(symlink($path."/local", $_SERVER['DOCUMENT_ROOT']."/local")) echo "Символические ссылки удачно созданы";
//					else
