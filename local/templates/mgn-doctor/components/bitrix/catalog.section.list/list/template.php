...
if (CModule::IncludeModule("iblock")) {
	$res = CIBlockSection::GetList(array(), array('IBLOCK_ID' => $arParams["IBLOCK_ID"], 'CODE' => $_REQUEST["SECTION_CODE"]));
	$section = $res->Fetch();

	$res = CIBlockSection::GetByID($section['IBLOCK_SECTION_ID']);
	if ($ar_res = $res->GetNext())
		$SectionID = $ar_res['CODE'];
}
?>
	...
	<ul class="all-services__list">
		...
		?>
			...
			?>
					...
					?>
					<li class="all-services__item" id="<?= $this->GetEditAreaId($arSection['ID']); ?>"><a href="<? echo $arSection["SECTION_PAGE_URL"]; ?>"><? echo $arSection["NAME"]; ?></a>
					<?	}
			} else {
					?>
